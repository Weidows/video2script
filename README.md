...
```bash
pip install -e ".[build]"
python scripts/make_icon.py        # regenerate the icon (already committed, optional)
python scripts/build_exe.py        # dist/video2script(.exe) — one binary, CLI + GUI
```
...
└── assets/        # icon.png / icon.ico (generated by scripts/make_icon.py)
```
